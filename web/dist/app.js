...
function setRwyActive(rwy, state) {
...
function updateAtisUI(atisData) {
    const out_atis = document.getElementById("atis");
    if (!out_atis)
        return;
    if (atisData) {
...
async function load_data(data, taf_data, atis_data) {
    ...
    if (!out_qnh || !wind_div || !out_wdir || !out_wspd || !out_conditions || !out_metar || !out_taf || !windArrow || !arrowHead)
        return;
    if (data && taf_data) {
        // Update data
